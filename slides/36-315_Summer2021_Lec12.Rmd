...
<br>
.medium[addressed time zones as much as possible]
...
<br>
.medium[same theme, fonts, colors, etc.]
<br>
...
```{r evaluate-13}
metrics <- function(dataset) {
  
  tibble(
    "log_loss" = 
      mn_log_loss(dataset, defense, .pred_D) %>%
      select(.estimate),
    "accuracy" = 
      accuracy(dataset, defense, .pred_class) %>%
      select(.estimate),
    "auc" = 
      roc_auc(dataset, defense, .pred_D) %>%
      select(.estimate)
  ) %>% 
    unnest(everything()) %>% 
...
metrics_compare <- 
  bind_rows(metrics_TOI, metrics_TOI_plus)
```
...
linear_predictions %>% 
  ...
  coord_equal() +
  cmu_theme() +
  annotate("text", x = 0, y = 2.5, label = "R^2 == 0.584", #<<
           parse = TRUE) + #<<
